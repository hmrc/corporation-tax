...
class AdjustmentTransactionsConnector @Inject() (http: HttpClientV2, config: ServicesConfig)(implicit
  ec: ExecutionContext
) extends Logging {
  ...
  def getAdjustmentTransactions(taxRef: Long, accPeriod: Long)(implicit
    ...
    val url: URL = url"$proxyPath/corporation-tax/adjustment-transactions/$taxRef/$accPeriod"

    http
      .get(url)
      .execute[AdjustmentTransactionsList]
      .recover { case ex: Throwable =>
        logger.error(
          s"[AdjustmentTransactionsConnector][getAdjustmentTransactionsList]: $taxRef :: $accPeriod - ${ex.getMessage}"
        )
