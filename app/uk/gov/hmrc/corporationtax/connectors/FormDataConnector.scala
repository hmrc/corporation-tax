...
import uk.gov.hmrc.corporationtax.models.CT600XmlDataResponse
...
class FormDataConnector @Inject() (http: HttpClientV2, appConfig: AppConfig)(implicit
  ec: ExecutionContext
) extends Logging {
  ...
  def getFormData(taxRef: Long, accPeriod: Long, startDate: LocalDate, endDate: LocalDate)(implicit
    ...
    val url: URL =
      url"${appConfig.rdsDatacacheProxyFullUrl}/ct-form-data/$taxRef/$accPeriod?startDate=${startDate.toString}&endDate=${endDate.toString}"
    http
      .get(url)
