...
class InterestChargesSummaryController @Inject() (
  ...
  def getInterestChargesSummary(taxPayerReference: String): Action[AnyContent] = Action.async { implicit request =>
    service
      ...
      .map { interestCharges =>
        Ok(Json.toJson(interestCharges))
      }
      .recover {
        case u: UpstreamErrorResponse =>
          Status(u.statusCode)(Json.obj("message" -> u.message))
        case t: Throwable             =>
          logger.error("Error while retrieving interestCharges", t)
          InternalServerError(Json.obj("error" -> "Failed to retrieve interestCharges"))
      }
  }
