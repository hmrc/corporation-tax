...
class PayRepayReallocationConnectorISpec
  ...
  "getTotalAmounts" should {
    ...
    "return a successful an empty payment repayment reallocation from BE" in {
      ...
      result mustBe emptyPayRepayReallocation
    }

    "return a payment repayment reallocation from BE" in {
      stubFor(
        get(urlPathEqualTo(url(1L, 5L)))
          .willReturn(
            aResponse()
      ...
      val result = connector.getTotalAmounts(1L, 5L).futureValue
      result mustBe payRepayReallocation
    }
        ...
        get(urlPathEqualTo(url(1L, 2L)))
          .willReturn(
            aResponse()
              ...
              .withBody(
                s"""{
                   |error" :"Failed to retrieve the payment repayment allocations from the BE"
                   |}""".stripMargin
              )
          )
      )
