...
class AdminRuleRdsProxyConnector @Inject() (http: HttpClientV2, config: ServicesConfig)(implicit
  ec: ExecutionContext
) extends Logging {
  ...
  def getAdminRule(adminRule: String)(implicit hc: HeaderCarrier): Future[AdminRule] = {
    val url: URL =
      if (stubEnabled) url"$stubPath/corporation-tax/administrative-rule/$adminRule"
      else url"$rdsDataCachePath/corporation-tax/administrative-rule/$adminRule"
    http
      .get(url)
      .execute[AdminRule]
      .recover {
        case e: UpstreamErrorResponse =>
          logger.error(
            s"[AdminRuleRdsProxyConnector][getAdminRule]: Upstream error - ${e.getMessage}"
          )
          throw e
        case e: Throwable             =>
          logger.error(
            s"[AdminRuleRdsProxyConnector][getAdminRule]: ${e.getMessage}"
          )
