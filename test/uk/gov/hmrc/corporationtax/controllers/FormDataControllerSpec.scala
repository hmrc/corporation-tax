...
import uk.gov.hmrc.corporationtax.connectors.FormDataConnector
import uk.gov.hmrc.corporationtax.helpers.FormDataHelper
import uk.gov.hmrc.corporationtax.queryParams.FormDataQueryParams
...
class FormDataControllerSpec extends AnyWordSpec with Matchers
  ...
  private trait Setup {
    ...
    val fakeRequest = FakeRequest("GET", "/ct-form-data")
    val controller  =
      new FormDataController(Helpers.stubControllerComponents(), mockFormDataConnector)
  }

  "GET /ct-form-data" should {

    "return 200 and a successful response with default record" in new Setup {
