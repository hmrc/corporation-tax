...
class DisplayNeededConnector @Inject() (http: HttpClientV2, appConfig: AppConfig)(implicit ec: ExecutionContext)
    extends Logging {

  def getDisplayNeeded(taxRef: Long, accPeriod: Long)(implicit hc: HeaderCarrier): Future[DisplayNeededResponse] = {
    val url: URL = url"${appConfig.rdsDatacacheProxyFullUrl}/display-needed/$taxRef/$accPeriod"
