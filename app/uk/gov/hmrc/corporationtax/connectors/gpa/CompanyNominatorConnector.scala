...
class CompanyNominatorConnector @Inject() (http: HttpClientV2, appConfig: AppConfig)(implicit
  ec: ExecutionContext
) extends Logging {

  def getIsCompanyNominatorOfGPA(gpaUtr: Long, nominatedCompanyUtr: Long)(implicit
    hc: HeaderCarrier
  ): Future[RdsCompanyNominator] = {
    val url: URL = url"${appConfig.rdsDatacacheProxyFullUrl}/is-company-nominator/$gpaUtr/$nominatedCompanyUtr"
